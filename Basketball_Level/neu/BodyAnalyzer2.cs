...
public class BodyAnalyzer2 : MonoBehaviour
{
	...
	private void calculateBallVelocity ()
	{
		...
		if (ball != null && ball.GetComponent<BallController2> ().BallTriggered == true) {
			...
			if (shoulderPos.y < currentHandPosition.y) {
				...
				if (isThrown == 2) {
					...
					ball.GetComponent<BallController2> ().BallTriggered = false;
					ball.GetComponent<BallController2> ().BallFlying = true;
